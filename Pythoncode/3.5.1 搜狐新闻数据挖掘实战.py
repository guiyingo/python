...
p_source = '<p class="news-from text-lightgray">.*?<span>(.*?)</span><span>.*?</span>.*?</p>'  #<p class="news-from text-lightgray">.*?<span>代码中的.*?估计主要是代替换行，否则
# p_source = '<p class="news-from text-lightgray">.*?<span>(.*?)</span>' #根据实际简化规则输出的结果也正常

...
def baidu(company):
    ...
    for i in range(len(title)):  # range(len(title)),这里因为知道len(title) = 10，所以也可以写成for i in range(10)
        ...
        print(str(i + 1) + '.' + title[i] + '(' + date[i][0] + '-' + source[i][0] + ')') # date 和source是列表，且列表里元素是列表。
        print(href[i])
    ...
    file1 = open('D:\\Github\\python\\数据挖掘报告.txt', 'a')  # 如果把a改成w的话，则每次生成txt的时候都会把原来的txt清空，用w不太好，因为这样只能保留一家公司信息；如果出现乱码问题，则设置encoding参数为utf-8，写成file1 = open('E:\\数据挖掘报告.txt', 'a'，encoding='utf-8')
    file1.write(company + '数据挖掘completed！' + '\n' + '\n')
    for i in range(len(title)):
        file1.write(str(i + 1) + '.' + title[i] + '(' + date[i][0] + '-' + source[i][0] + ')' + '\n')
        file1.write(href[i] + '\n')  # '\n'表示换行
    file1.write('——————————————————————————————' + '\n' + '\n')

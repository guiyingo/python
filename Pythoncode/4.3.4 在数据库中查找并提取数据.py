...
db = pymysql.connect(host='36.137.244.206', port=3308, user='python', password='179180', database='python', charset='utf8')

company = '阿里巴巴'

cur = db.cursor()  # 获取会话指针，用来调用SQL语句
sql = 'SELECT * FROM python WHERE company = %s'  # 编写SQL语句
...
cur = db.cursor()  # 获取会话指针，用来调用SQL语句
sql = 'SELECT * FROM python WHERE company = %s or title = %s'  # 编写SQL语句
cur.execute(sql, (company, title))  # 执行SQL语句
data = cur.fetchall()  # 提取所有数据，并赋值给data变量,结果是字典。
print(data)
db.commit()  # 这个其实可以不写，因为没有改变表结构
cur.close()  # 关闭会话指针
db.close()  # 关闭数据库链接
